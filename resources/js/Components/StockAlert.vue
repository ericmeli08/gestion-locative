<template>
  <div class="flex items-center p-4 border rounded-lg" :class="alertClasses">
    ...
    <div class="ml-4">
      ...
      <Link :href="'#'" class="text-sm font-medium underline hover:no-underline">
        {{ $t('stocks.restock') }}
      </Link>
    </div>
  </div>
...
const alertClasses = computed(() => {
  const classes = {
    critical:
      'border-error-200 bg-error-50 text-error-700 dark:border-error-800 dark:bg-error-900/50 dark:text-error-400',
    warning:
      'border-warning-200 bg-warning-50 text-warning-700 dark:border-warning-800 dark:bg-warning-900/50 dark:text-warning-400',
    normal:
