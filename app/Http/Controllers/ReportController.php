...
class ReportController extends Controller
{
    public function monthly(Request $request)
    {
        ...
        if ($apartmentId) {
            // ── Un seul appartement : données agrégées par mois (ancienne logique) ──
            ...
            // ── Tous les appartements : données par appartement ET par mois ───
            // Chaque entrée = { month, apartment_id, apartment_name, revenue, expenses, charges, profit, margin }
            while ($current <= $endDate) {
                $monthStart = $current->copy()->startOfMonth();
                $monthEnd   = $current->copy()->endOfMonth();

                foreach ($apartments as $apartment) {
                    $monthData = $this->getMonthData($monthStart, $monthEnd, $apartment->id);

                    $monthData['month']          = $current->format('Y-m');
                    $monthData['apartment_id']   = $apartment->id;
                    $monthData['apartment_name'] = $apartment->name;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  Données d'un mois pour un appartement donné
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  Données d'un jour pour un appartement donné
    // ─────────────────────────────────────────────────────────────────────────
    private function getDayData($start, $end, $apartmentId)
    {
        ...
        // Les charges mensuelles ne sont pas calculées par jour
        $charges = 0;
        ...
        while ($current <= $endDate) {
            ...
            foreach ($apartments as $apartment) {
                ...
                $occupancyRate = $daysInMonth > 0
                    ? round(($occupiedDays / $daysInMonth) * 100, 1)
                    : 0;
